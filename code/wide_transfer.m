...
checkpointDir = 'modelCheckpoints_wide_Aug_transfer';
...
%     convolution2dLayer(5,80,'Padding',[2 2],'Stride', [1,1]);  % convolution layer with 20, 5x5 filters
...
%     %reluLayer();  % ReLU layer
%     %maxPooling2dLayer(2,'Stride',2); % Max pooling layer
%     convolution2dLayer(3,120,'Padding',[1 1],'Stride', [1,1]);  % convolution layer with 20, 5x5 filters
%     reluLayer();  % ReLU layer
%     maxPooling2dLayer(2,'Stride',2); % Max pooling layer
% 
...
filename = strcat('Wd_Transfer_Train_confusion_matrix_',num2str(numEpochs),'.xlsx');
xlswrite(filename, train_conf_mat,1,'A1');

filename = strcat('Wd_TransferTrain_classification_matrix_',num2str(numEpochs),'.xlsx');
...
filename = strcat('Wd_Transfer_Validation_confusion_matrix_',num2str(numEpochs),'.xlsx');
xlswrite(filename, val_conf_mat,1,'A1');

filename = strcat('Wd_Transfer_Validation_classification_',num2str(numEpochs),'.xlsx');
...
filename = strcat('Wd_Transfer_Test_confusion_matrix_',num2str(numEpochs),'.xlsx');
xlswrite(filename, test_conf_mat,1,'A1');

filename = strcat('Wd_Transfer_Test_classification_matrix_',num2str(numEpochs),'.xlsx');
xlswrite(filename, test_class_mat,1,'A1');

%%
f=figure;
plotTrainingAccuracy_All(info1,numEpochs);
saveas(f,'Wd_Transfer_wide_10Epoc.png');
